...
select
  ...
  case
    when e.kind = 'table'
      then case when to_regclass('public.' || e.name) is not null then '✅ bor' else '❌ YO''Q' end
    ...
    when e.kind = 'constraint'
      then case when exists (
             select 1 from pg_constraint where conname = e.name
           ) then '✅ bor' else '❌ YO''Q' end
  end as holat
from expected e
order by
  case when holat like '❌%' then 0 else 1 end,  -- yo'qlari yuqorida
  e.migration,
  e.kind,
  e.name;
